...
def get_valid_ids(cursor):
    """Lấy danh sách các ID hợp lệ từ database"""
    # Lấy warehouse_id
    ...
    # Lấy tất cả category_id thực sự tồn tại
...
def random_product_data(valid_ids):
    """Tạo dữ liệu ngẫu nhiên cho sản phẩm"""
    product_names = [
    ...
    # Random các giá trị khác với foreign key hợp lệ
    category_id = random.choice(valid_ids['category_ids'])
...
def main():
    ...
    try:
        ...
        print(f"  - Có {len(valid_ids['category_ids'])} categories")
